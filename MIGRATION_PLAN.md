...
- ✅ 比較用スクリプト`scripts/canary_compare.py`を準備済み・構造テスト完了（2026-07-13、`VERIFICATION_LOG.md`参照）。I2C/SPI系センサーはdirect/pi4gpio両方を並行読み取りしCSVへ記録、GPIO/UART/DHT22系（本番と競合しうる）はpi4gpioモードのみ読み取り本番の`journalctl`ログと突き合わせる設計
- ✅ センサー再接続後の実データ比較を実施（2026-07-13）。本番停止中の一時的な検証で全7センサーの実データ突き合わせが完了し、DHT22の重大バグを発見・修正した
- ✅ `canary-compare.service`としてsystemd常駐化し、本番稼働と並行した1〜2週間規模の本格運用を開始（2026-07-13 12:34 JST〜）。Pi再起動を挟んでも自動再開する。初回記録でDHT22（pi4gpio 25.3℃/64.4% vs 本番参考値25.2℃/63.8%）・MHZ19C CO2（pi4gpio 610ppm vs 本番参考値607ppm）とも妥当な近似値を確認済み。1〜2週間後にCSVを評価し、次項（本番の段階的cutover）の判断材料とする
