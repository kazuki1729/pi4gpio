...
## 2026-07-23: 正式24時間direct基準の取得

本番サービス、設定、ハードウェアを変更せず、Piのjournalと受信サーバーのSQLite正本を読み取り専用で集計した。SQLiteはURIの`mode=ro`と`PRAGMA query_only=ON`を併用した。

### 実行前安全確認

- week09: PID 1253、`active/running`、`NRestarts=0`、`ExecMainStatus=0`
- pi4gpiod: PID 967、`active/running`、`NRestarts=0`、`ExecMainStatus=0`
- sensor-server: PID 223299、`active/running`、`NRestarts=0`、`ExecMainStatus=0`
- カナリア: `not-found/inactive`
- week09の`RPI_SENSOR_BACKEND`指定とdrop-inはなく、10秒direct運転を維持
- Pi上のjournal解析スクリプトとローカルのSHA-256が一致
- 本番停止・再起動、周期変更、ハードウェア操作、障害注入は実施していない

### 正式基準値

- 判定条件: 24時間指定に対し実時間23.99時間以上
- journal: 2026-07-22 10:20:30～2026-07-23 10:20:26、23.999時間、8,636件
- SQLite: 2026-07-22 10:21:20～2026-07-23 10:21:16、23.999時間、8,636件
- timer 8,636件、button 0件、平均周期10.005秒、最小10秒、最大11秒、10秒±0.5秒外46件
- light/sound/joystick/potentiometer/DHT22/BME280/MH-Z19Cは全て8,636/8,636件有効（100%）
- journalのセンサー失敗周期0件、各対象センサー失敗言及0件、通信エラー0件
- 値域: DHT22 28.1～32.5℃、湿度56.2～69.7%、気圧1005.374～1008.71hPa、CO2 400～527ppm
- 両経路はレコード数、周期統計、有効レコード数、値域が一致。取得時刻差50秒による平均値の丸め差は最大0.002
